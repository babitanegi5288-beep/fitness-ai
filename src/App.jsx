...
function TrainingPage({ profile }) {
  ...
  async function loadExercises() {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/training/exercises?sport=${encodeURIComponent(
          profile.sport
...
function NutritionPage({ profile }) {
  ...
  async function loadTodayFood() {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/nutrition/food/${profile.id}`
      );
...
Dinner: ${foodEntry.dinner}
    ...
    try {
      const response = await fetch(
        "http://127.0.0.1:8000/api/nutrition/analyze",
        {
          method: "POST",
...
function FoodScannerPage({ profile }) {
  ...
  async function scanFood() {
    ...
    try {
      ...
      const response = await fetch(
        "http://127.0.0.1:8000/api/food-scanner/analyze",
        {
          method: "POST",
          body: formData,
        }
...
function AICoachPage({ profile }) {
  ...
  useEffect(() => {
    async function loadHistory() {
      try {
        ...
        const response = await fetch(
          `http://127.0.0.1:8000/api/ai-coach/history/${userId}`
        );
  ...
  async function sendMessage() {
    ...
    try {
      ...
      const response = await fetch(
        "http://127.0.0.1:8000/api/ai-coach/ask",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
...
function SettingsPage({
  ...
  async function saveSettings() {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/auth/profile/${profile.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
